...
export const MapComponent = forwardRef<HTMLDivElement, MapComponentProps>(
  (
    ...
    const getPopupPosition = (markerX: number, markerY: number, popupWidth: number, popupHeight: number) => {
      ...
      const offsetY = 0 // Default vertical offset from marker
      ...
      const wouldExtendBottom = markerY + offsetY + popupHeight > mapDimensions.height - margin
    ...
    return (
      <div
        ...
        className={`relative w-full h-full overflow-hidden rounded-lg ${
          selectedDatacenterType ? "cursor-crosshair" : "cursor-default"
        }`}
        ...
        {datacenters.map((datacenter) => {
          ...
          return (
            <div key={datacenter.id} className="group z-10" style={{ position: "absolute", left: displayX, top: displayY }}>
              <div
                className={`w-7 h-7 ${getDatacenterColor(datacenter)} rounded-lg border-2 shadow-lg cursor-pointer transform -translate-x-1/2 -translate-y-1/2 transition-all hover:scale-105 flex items-center justify-center ${
                ...
                } ${selectedDatacenter === datacenter.id && gameStarted ? "cursor-move" : ""} ${
                  isBeingDragged ? "scale-125 shadow-2xl z-50" : ""
                }`}
                onClick={(e) => {
                  e.stopPropagation()
              ...
              {(() => {
                ...
                return (
                  <div
                    className={`bg-white px-3 py-2 rounded-lg shadow-lg text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity z-50 border max-w-48 ${
        ...
        {mappedPowerGenerators.map((generator) => {
          ...
          return (
            <div key={generator.id} className="absolute group z-10" style={{ left: validX, top: validY }}>
              <div
                className={`w-5 h-5 rounded-full border-2 border-white shadow-lg cursor-pointer transform -translate-x-1/2 -translate-y-1/2 hover:scale-110 transition-transform ${
                  generator.type === "coal"
                    ? "bg-gray-600"
                    : generator.type === "gas"
                      ? "bg-orange-500"
                  ...
                  generator.powerLevel > 100 ? "ring-2 ring-yellow-400" : ""
                }`}
                onClick={(e) => {
                  e.stopPropagation()
                  if (gameStarted) {
          ...
          <div className="absolute pointer-events-none z-10" style={{ left: hoverPreview.x, top: hoverPreview.y }}>
            {(() => {
              ...
              return (
                <>
                  <div
                    className={`w-7 h-7 rounded-lg border-2 shadow-lg transform -translate-x-1/2 -translate-y-1/2 opacity-70 animate-pulse flex items-center justify-center ${
                      selectedDatacenterType === "tier1"
                        ? "bg-green-500"
